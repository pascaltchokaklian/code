...
from StravaMap.models import Activity, Activity_info, Col_perform, Month_stat, Perform, Region, Segment, User_dashboard, User_var
...
def connected_map(request):
    ...
    if len(activities_df)>0: 
        ...
        for ligne in range(len(activities_df)):
            ...
            my_Activity_info.act_id = 1
            my_Activity_info.info_txt = get_last_activity_more_than(strava_user_id,act_dist)           
            my_Activity_info.save()

            get_last_activity_den_than(strava_user_id,act_den)       
            get_last_speed_activity(strava_user_id,act_dist,act_time)    
