...
from StravaMap.vars import display_year_month, f_debug_col, f_debug_trace
...
def get_last_activity_more_than(strava_user_id, distance):
    ...
    for one_act in Activity.objects.filter(strava_user_id=strava_user_id).order_by("-act_start_date"):
        if one_act.act_dist > distance: 
            print('Plus grande sortie depuis "',one_act.act_name, '"le' ,  one_act.act_start_date )
            display_year_month(one_act.act_start_date)
